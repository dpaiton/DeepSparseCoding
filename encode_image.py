...
if "rand_seed" in model_params.keys():
...
## Import data
data = {"train": Dataset(model_params["data_file"], model_params)}
...
with tf.Session(config=config, graph=model.graph) as sess:
  ...
  sess.run(model.init_op,
    feed_dict={model.x:np.zeros([model.params["batch_size"]]+model.params["data_shape"], dtype=np.float32)})

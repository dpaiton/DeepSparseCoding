...
class ReconAdversarialModule(object):
  def __init__(self, data_tensor, use_adv_input, num_steps, step_size, adv_upper_bound=None,
    clip_adv=True, clip_range=[0.0, 1.0], carlini_change_variable=False, optimizer="sgd",
    variable_scope="latent_adversarial"):
    """
    Adversarial module
    Inputs:
      ...
      optimizer: [str] specifying "sgd" or "adam" for the carlini optimizer
  ...
  def build_init_graph(self):
    with tf.variable_scope(self.variable_scope) as scope:
      ...
      with tf.variable_scope("placeholders") as scope:
        self.selection_vector = tf.placeholder(tf.float32, shape=self.input_shape,
      ...
      with tf.variable_scope("input_var"):
        ...
        if(self.adv_upper_bound is not None):
          self.clipped_pert = tfc.upper_bound(tfc.lower_bound(
            carlini_change_pert, -self.adv_upper_bound), self.adv_upper_bound)
        else:
    ...
    with tf.variable_scope(self.variable_scope) as scope:
      ...
      with tf.variable_scope("optimizer") as scope:
        if(self.optimizer == "adam"):
          ...
          assert False, ("optimizer must be 'adam' or 'sgd'")

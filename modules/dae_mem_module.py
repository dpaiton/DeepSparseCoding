...
class DaeMemModule(DaeModule):
  def __init__(self, data_tensor, output_channels, ent_mult, decay_mult, bounds_slope, latent_min,
    latent_max, num_triangles, mle_step_size, num_mle_steps, gdn_w_init_const, gdn_b_init_const,
    gdn_w_thresh_min, gdn_b_thresh_min, gdn_eps, memristor_data_loc, memristor_type, memristor_std_eps, 
    synthetic_noise, mem_error_rate, act_funcs, dropout, tie_decoder_weights, conv=False, conv_strides=None, patch_y=None, 
    patch_x=None, variable_scope="dae_mem"):
    """
    ...
    super(DaeMemModule, self).__init__(data_tensor, output_channels, ent_mult, decay_mult, bounds_slope, latent_min,
    latent_max, num_triangles, mle_step_size, num_mle_steps, num_quant_bins, noise_var_mult, gdn_w_init_const, gdn_b_init_const,
    gdn_w_thresh_min, gdn_b_thresh_min, gdn_eps, act_funcs, dropout, tie_decoder_weights, conv, 
    conv_strides, patch_y, patch_x, variable_scope)
  ...
  def build_graph(self):
    with tf.variable_scope(self.variable_scope) as scope:
      ...
      u_in_shape = self.u_list[-1].get_shape().as_list()
      if self.conv:
        self.num_latent = np.prod(u_in_shape[1:])
      else:

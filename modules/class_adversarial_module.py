...
class ClassAdversarialModule(object):
  def __init__(self, data_tensor, num_classes, num_steps, step_size, max_step=None, clip_adv=True,
  ...
  def build_init_graph(self):
    ...
    with tf.name_scope("input_var"):
      ...
      reshape_adv_var = tf.reshape(self.adv_var, [-1,] + self.input_shape[1:])

      #Clip pertubations by maximum amount of change allowed
      if(self.max_step is not None):
        max_pert = tfc.upper_bound(tfc.lower_bound(
          reshape_adv_var, -self.max_step), self.max_step)
      else:
        max_pert = reshape_adv_var
  ...
  def build_adversarial_ops(self, label_est, label_tensor=None, model_logits=None):
    self.label_est = label_est
    with tf.name_scope("loss") as scope:
      #TODO this is assuming cross entropy loss, can we remove this assumption?
      if(self.attack_method == "kurakin_untargeted"):
        self.adv_loss = tf.reduce_sum(tf.multiply(self.label_est,
          tf.log(tf.clip_by_value(label_tensor, self.eps, 1.0))))
      elif(self.attack_method == "kurakin_targeted"):
        self.adv_loss = -tf.reduce_sum(tf.multiply(self.label_est,
          tf.log(tf.clip_by_value(self.adv_target, self.eps, 1.0))))
  ...
  def gen_feed_dict(self, feed_dict, use_adv_input=True, labels=None, recon_mult=None,
  ...
  def construct_adversarial_examples(self, feed_dict, save_int=None):
    ...
    return feed_dict, adversarial_images, adversarial_output

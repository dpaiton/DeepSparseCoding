...
def plot_iso_contour_set(analysis_dict, curvatures, num_levels, num_x, num_y, show_contours=True, text_width=200, width_fraction=1.0, dpi=100):
    arrow_width = 0.0
    arrow_linewidth = 1
    arrow_headsize = 0.15
    ...
    if num_y > analysis_dict['activations'].shape[0]: # num target neurons
        num_y = analysis_dict['activations'].shape[0]
    if num_x > analysis_dict['activations'].shape[1]: # num comparison planes
        num_x = analysis_dict['activations'].shape[1]
    num_plots_y = num_y
    num_plots_x = num_x
    gs0 = gridspec.GridSpec(num_plots_y, num_plots_x, wspace=gs0_wspace, hspace=gs0_hspace)
    vmin = -1.0#np.min(analysis_dict['activations'])
    vmax = 1.0#np.max(analysis_dict['activations'])
    ...
    for plot_id in np.ndindex((num_plots_y, num_plots_x)):
        ...
        j_idx = analysis_dict["comparison_neuron_ids"][neuron_index][orth_index]
...
def plot_group_iso_contours(analyzer_list, neuron_indices, orth_indices, num_levels, x_range, y_range, show_contours=True,
  ...
  for plot_id in np.ndindex((num_plots_y, num_plots_x)):
    if analyzer_index < num_models:
      ...
      contour_handles.append(contsf)
      # plot target neuron arrow & label
      proj_target = analyzer.comp_contour_dataset['proj_target_vect'][analyzer_neuron_index][analyzer_orth_index]
      target_vector_x = proj_target[0].item()
      target_vector_y = proj_target[1].item()
      curve_axes[-1].arrow(0, 0, target_vector_x, target_vector_y,
        width=arrow_width, head_width=arrow_head_width, head_length=arrow_head_length,
      ...
      text_handle = curve_axes[-1].text(
        ...
        r'$\Phi_{k}$', horizontalalignment='center', verticalalignment='center')
      # plot comparison neuron arrow & label
      proj_comparison = analyzer.comp_contour_dataset['proj_comparison_vect'][analyzer_neuron_index][analyzer_orth_index]
      comparison_vector_x = proj_comparison[0].item()
      comparison_vector_y = proj_comparison[1].item()
      curve_axes[-1].arrow(0, 0, comparison_vector_x, comparison_vector_y,
        width=arrow_width, head_width=arrow_head_width, head_length=arrow_head_length,
      ...
      #for proj_alt in analyzer.comp_contour_dataset['proj_comparison_vect'][analyzer_neuron_index]:
      ...
      if curvature[analyzer_index] is not None:
        curve_axes[-1].text(x=-0.08, y=1.75, s=f'C={curvature[analyzer_index]:.3f}',
          horizontalalignment='right', verticalalignment='center', fontsize=6) 
...
def plot_circ_variance_histogram(analyzer_list, circ_var_list, color_list, label_list, num_bins, density, width_ratios,
  ...
  axes[-1].grid(b=False, which='both', axis='both')
  variance_min = 0.0
  variance_max = 1.0
  ...
  for analyzer_idx, analyzer in enumerate(analyzer_list):
    ...
    for plot_id in np.ndindex((num_plots_y, num_plots_x)):
      ...
      (y_id, x_id) = plot_id
      if y_id == 0 and x_id == 0:
        plt.text(x=0.1, y=1.45, s=label_list[analyzer_idx], horizontalalignment='center',

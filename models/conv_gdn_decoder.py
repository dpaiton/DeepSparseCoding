...
class Conv_GDN_Decoder(GDN_Autoencoder):
  ...
  def load_params(self, params):
    ...
    ######init decoder########
    # self.w_init_list = params["w_init_list"]
    # self.b_init_list = params["b_init_list"]
    # self.w_igdn_init_list = params["w_igdn_init_list"]
    # self.b_igdn_init_list = params["g_igdn_init_list"]
    ##########################
  ...
  #   r = mem_utils.memristor_output(v_clip,  , vs_data, mus_data, sigs_data,
    ...
    with tf.variable_scope(self.weight_scope) as scope:
      if inverse:
        w_gdn = tf.get_variable(name="w_igdn"+str(layer_id), shape=self.w_gdn_shapes[layer_id],
          dtype=tf.float32, initializer=tf.constant(self.w_igdn_init_list[layer_id], dtype=tf.float32), trainable=False)
        b_gdn = tf.get_variable(name="b_igdn"+str(layer_id), shape=self.b_gdn_shapes[layer_id],
          dtype=tf.float32, initializer=tf.constant(self.b_igdn_init_list[layer_id], dtype=tf.float32), trainable=False)
      # else:
  ...
  def layer_maker(self, layer_id, u_in, w_shape, w_stride, decode):
    ...
    with tf.variable_scope(self.weight_scope) as scope:
      w = tf.get_variable(name="w"+str(layer_id), shape=w_shape, dtype=tf.float32,
        initializer=tf.constant(self.w_init_list[layer_id], dtype=tf.float32), trainable=False)
    ...
    with tf.variable_scope(self.weight_scope) as scope:
      b = tf.get_variable(name="b"+str(layer_id), shape=w_shape[bias_shape_id],
        dtype=tf.float32, initializer=tf.constant(self.b_init_list[layer_id], dtype=tf.float32), trainable=True)
  ...
  def build_graph(self):
    ...
    with tf.device(self.device):
      with self.graph.as_default():
        ...
        #   # initialize these from npz file 
        #   self.w_init = tf.constant(w,dtype=tf.float32)
        #   self.b_init = tf.constant(b,dtype=tf.float32)
        #   self.w__igdn_init = tf.constant(w_igdn,dtype=tf.float32)
        #   self.b_igdn_init = tf.constant(b_igdn,dtype=tf.float32)

  ...
  def cov_analysis(self, dataset, save_info=""):
    with tf.Session(graph=self.model.graph) as sess:
      sess.run(self.model.init_op,
        feed_dict={self.model.x:np.zeros([self.model.batch_size, self.model.num_pixels],
        dtype=np.float32)})
      self.model.load_weights(sess, self.cp_loc)
      act_cov = None
      num_cov_in_avg = 0
      while dataset.curr_epoch_idx + self.model.batch_size < dataset.num_examples:
        if act_cov is not None and num_cov_in_avg == 10: # prevent numerical overflow
          act_cov /= num_cov_in_avg
          num_cov_in_avg = 0
        input_data = dataset.next_batch(self.model.batch_size)[0]

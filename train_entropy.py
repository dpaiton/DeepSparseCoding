...
if data_type.lower() == "vanhateren":
  ...
  if params.vectorize_data:
    params.data_shape = [paramspatch_edge_size**2]
  else:
    params.data_shape = [params.im_size_y, params.im_size_x, 1]
  data = {"train": Dataset(params)}
  params.num_batches = (params.schedule[0]["num_epochs"]
    * data["train"].num_examples) / params.batch_size
  params.schedule[0]["decay_steps"] = [int(0.8*params.num_batches)
    for _ in range(len(params.schedule[0]["weights"]))]
      ...
      if model.model_type== "conv_gdn_autoencoder":
        if model.params.memristor_type == "rram":
          mem_std_eps = np.random.uniform(low=-1.0, high=1.0,
             size=(model.batch_size,  model.n_mem)).astype(np.float32)
        else:
